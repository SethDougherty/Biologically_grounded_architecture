...
def compute_features(voltage_trace):
    ...
    feature_values_list = efel.getFeatureValues([trace1], features_to_compute)
...
def compute_DTWD(volts_actual, volts_predict):
    ...
    for i in range(volts_actual.shape[0]):
        ...
        try:
            ...
            print(f"DTW failed for trace {i}: {e}")
    print("First 10 DTW distances:", dtw_distances[:10])
    return dtw_distances
...
def save_to_csv(csv_save_path, total_mse_sum, dynamic_time_warp_distance, csv_name):
    ...
    file_path = os.path.join(csv_save_path, csv_name)
    df.to_csv(file_path)
    print(f"Saved error CSV to: {file_path}")
...
def main(path1, path2, csv_name, stim_index):
    ...
    print("volts_actual min/max:", volts_actual.min(), volts_actual.max())
    print("volts_predict min/max:", volts_predict.min(), volts_predict.max())
    ...
    save_to_csv(save_dir,total_mse_sum,dynamic_time_warp_distance,filename)
    # save_to_csv(os.path.dirname(csv_path), total_mse_sum, dynamic_time_warp_distance, os.path.basename(csv_path))
    # plot_violin(total_mse_sum)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Process two directories of HDF5 files.')
    parser.add_argument('--path1', type=str,required=False,default ="/pscratch/sd/k/ktub1999/BBP_TEST1/runs2/29389166_1/L5_TTPC1_cADpyr232_1/c1" , help='Path to the first directory containing HDF5 files (predicted volts)')
    parser.add_argument('--path2', type=str,required=False,default ="/pscratch/sd/k/ktub1999/BBP_TEST1/runs2/29389166_1/L5_TTPC1_cADpyr232_1/c1", help='Path to the second directory containing HDF5 files (actual volts)')
